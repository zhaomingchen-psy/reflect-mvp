...
- 先说接住了什么，再说遗漏了什么。判 partial 时必须先肯定他已经接住的那一层，再指出还差哪一层。
- 判 miss 时点出偏离类型（建议/宽慰/解释/提问）并说明此刻它为何会让来访者落空；对偏离行为的描述必须与其实际功能相符（例如"然后呢"这类追问是邀请对方继续说，不是打断，不要写成打断）。
...
{{"is_reflection": true/false, "matched": ["<对上的候选项原文，逐项列出，没有则空列表>"], "verdict": "hit"|"partial"|"miss", "captured": "<接住了什么，没有则空字符串>", "missed": "<遗漏或偏离了什么；matched 非空时必须为空字符串>", "comment": "<点评，60字以内>", "rewrite_hint": "<改写方向，40字以内，不给完整句子>"}}"""
